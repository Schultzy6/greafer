...
module.exports = {
  ...
  data: new SlashCommandBuilder().setName('sale').setDescription('Input a new sale'),

  // The actual code to be executed every time the command is invoked
  async execute(interaction) {
    const modal = new ModalBuilder().setCustomId('saleModal').setTitle('Sales:\n(Only use whole numbers)');
    ...
    const upgradesSold = new TextInputBuilder()
      ...
      .setLabel('Upgrades/Flat rate sold?')
      .setStyle(TextInputStyle.Short)
      .setPlaceholder('0')
      .setValue('0')
      .setRequired(false);
